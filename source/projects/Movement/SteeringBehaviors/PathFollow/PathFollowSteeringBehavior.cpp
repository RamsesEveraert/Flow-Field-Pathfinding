...
SteeringOutput PathFollow::CalculateSteering(float deltaT, SteeringAgent* pAgent)
{
	...
	if (m_currentPathIndex < static_cast<int>(m_pathVec.size()))
	{
...
{
	return m_currentPathIndex >= static_cast<int>(m_pathVec.size());
}

void PathFollow::GotoNextPathpoint()
{
	++m_currentPathIndex;
	if (m_currentPathIndex >= static_cast<int>(m_pathVec.size())) return;

	if (m_currentPathIndex == static_cast<int>(m_pathVec.size()) -1)
	{
